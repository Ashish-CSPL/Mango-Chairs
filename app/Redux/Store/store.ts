...
const rootReducer = (state: any, action: any) => {
    ...
    if (action.type === logout.type) {
        ...
        storage.removeItem('persist:root')
            .then(() => {
                console.log("STORE: Successfully removed 'persist:root' from localStorage.");
            })
            .catch(error => {
                console.error("STORE: Error removing 'persist:root' from localStorage:", error);
            });

        // Reset the entire Redux state to undefined.
        // This tells combineReducers to re-initialize all slices to their initialState.
        state = undefined;
        console.log("STORE: Redux state successfully reset to undefined by rootReducer.");
    }
...
export const persistor = persistStore(store, {}, () => {
    ...
    console.log("STORE: Current user state after rehydration:", store.getState().auth.user);
});
